...
`Manifest payload: passed`, and a checked `Expected certificate SHA-256`; it
also expects `Strict artifacts: true` plus the same APK and SBOM artifact names
from the release artifact verification report.
...
a reviewer, review packet, checked item count, `Unchecked items: 0`, and
`Issues: 0`.
